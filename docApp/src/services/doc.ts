...
export async function getPreviewUrlByFileName(fileName: string) {
  return request<{ code: number; data: string; message: string }>(`/api/doc/preview/fileName/${fileName}`, {
    method: 'GET',
  });
}
...
export async function batchExtractAndImport() {
  return request<{ code: number; data: string; message: string }>('/api/doc/batch/extract-import', {
    method: 'POST',
  });
}
...
export async function getBatchExtractImportStatus(taskId: string) {
  return request<{ code: number; data: any; message: string }>(
    `/api/doc/batch/extract-import/status/${taskId}`,
    {
      method: 'GET',
    },
  );
