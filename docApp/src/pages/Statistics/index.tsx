...
import { getStatistics, getDocCountByCategory, getDocCountBySeries } from '@/services/statistics';
import {
  BookOutlined,
  FileOutlined,
  EditOutlined,
...
const StatisticsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategoryData = async () => {
      try {
        const result = await getDocCountByCategory();
    ...
    const fetchSeriesData = async () => {
      try {
        const result = await getDocCountBySeries();
  ...
  const indexRatio = stats.fileCount > 0 
    ? Math.round((stats.indexedDocCount / stats.fileCount) * 100) 
    : 0;
                    ...
                    <Row gutter={[24, 24]}>
                        <Col xs={12}>
                            <StatCard
                                ...
                                color="#1890ff"
                                bgColor="#e6f7ff"
                                // secondaryValue={stats.docCount > 0 ? `近30天: ${Math.round(stats.docCount * 0.3)}` : '0'}
                                // secondaryText="新增书籍"
                                />
                        </Col>
                        <Col xs={12}>
                    ...
                    <Row gutter={[24, 24]}>
                        ...
                        <Col xs={12}>
                            <StatCard
                                ...
                                color="#fa8c16"
                                bgColor="#fff7e6"
                                // secondaryValue={stats.noteCount > 0 ? `平均每书: ${(stats.noteCount / (stats.docCount || 1)).toFixed(1)}` : '0'}
                                // secondaryText="笔记密度"
                            />
                        </Col>
                    </Row>
