...
import { searchDocs, queryDocList, getDocCoverByImgId, getCategories } from './service';
...
const DocCover: FC<DocCoverProps> = ({ picUrl = '', title, docId = null }) => {
  const [base64Data, setBase64Data] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const fetchDocCover = async () => {
      if (picUrl?.startsWith('img/') && docId) {
        const imgId = picUrl.split('/')[1];
        setLoading(true);
        try {
          const response = await getDocCoverByImgId(imgId);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await (searchKeyWord 
        ? searchDocs({ ...params, keyword: searchKeyWord }) 
        : queryDocList({ ...params, title: searchKeyWord }));
