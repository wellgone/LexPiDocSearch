...
public class TopicServiceImpl extends ServiceImpl<TopicMapper, Topic> implements TopicService {
    ...
    @Autowired
    private DocTopicMapper docTopicMapper;
    ...
    @Override
    public List<Topic> getTopicsByDocId(Long docId) {
        // 1. 获取书籍关联的所有主题ID
        LambdaQueryWrapper<DocTopic> docTopicWrapper = new LambdaQueryWrapper<>();
        docTopicWrapper.eq(DocTopic::getDocId, docId)
                .eq(DocTopic::getIsDeleted, 0);
        List<DocTopic> docTopics = docTopicMapper.selectList(docTopicWrapper);
        ...
        // 2. 获取这些主题的详细信息
        Set<Long> topicIds = docTopics.stream()
                .map(DocTopic::getTopicId)
    ...
    @Override
    public List<TopicTreeDTO> getTopicTreeByDocId(Long docId) {
        // 1. 获取所有相关主题
        List<Topic> allTopics = getTopicsByDocId(docId);
    ...
    @Override
    public List<TopicPathDTO> getDocTopicPaths(Long docId) {
        // 1. 获取书籍关联的所有主题ID
        LambdaQueryWrapper<DocTopic> docTopicWrapper = new LambdaQueryWrapper<>();
        docTopicWrapper.eq(DocTopic::getDocId, docId)
                .eq(DocTopic::getIsDeleted, 0);
        ...
        for (DocTopic docTopic : docTopics) {
            List<Topic> topicPath = getTopicWithParents(docTopic.getTopicId());
            ...
            if (!topicPath.isEmpty()) {
                ...
                pathDTO.setTopicId(docTopic.getTopicId());
                // 使用 > 连接主题名称
    ...
    @Override
    public List<Topic> getDocTopicTags(Long docId) {
        return baseMapper.getDocTopicTags(docId);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean parseDocOPACTopics(Long docId, String topicContent) {
        if (org.apache.commons.lang3.StringUtils.isBlank(topicContent)) {
            logger.info("书籍[{}]的主题词为空，跳过处理", docId);
            return false;
        }
        
        logger.info("开始解析书籍[{}]的主题词: {}", docId, topicContent);
        ...
        docTopic.setTopicId(rootParentId);
        docTopicMapper.insert(docTopic);  
        ...
        for (String topicGroup : topicGroups) {
            ...
            for (int i = keywords.length - 1; i >= 0; i--) {
                ...
                // DocTopic docTopic = new DocTopic();
                // docTopic.setDocId(docId);
                // docTopic.setTopicId(topic.getId());
                
                // 查询是否已存在关联
                LambdaQueryWrapper<DocTopic> docTopicQuery = new LambdaQueryWrapper<>();
                docTopicQuery.eq(DocTopic::getDocId, docId)
                            .eq(DocTopic::getTopicId, topic.getId());
                Long count = docTopicMapper.selectCount(docTopicQuery);
                
                // 不存在则添加关联
                if (count == 0) {
                    docTopicMapper.insert(docTopic);
                    logger.info("书籍[{}]关联主题: {}", docId, topic.getId());
                }
            }
        }
