...
public class FileServiceImpl implements FileService {
     ...
     */
    private Doc getDuplicateDocInfo(String fileName) {
        try {
            //根据文件名获取文件信息
            ...
            DocFileDTO lpDocFileDTO = docFileService.getDocFilesNoDeleteByDocId(fileId);
            if (lpDocFileDTO == null) {
                return null;
            }
            Doc lpDoc = docService.getById(lpDocFileDTO.getDocId());
            if (lpDoc == null) {
                return null;
            }
            return lpDoc;
    ...
    public FileUploadResult uploadFile(MultipartFile file) {
        try {
            ...
            if (existingFileName != null) {
                ...
                // 获取重复文件对应的图书信息
                Doc duplicateDoc = getDuplicateDocInfo(existingFileName);
                if (duplicateDoc != null) {
                    log.info("找到重复文件对应的图书: ID={}, 标题={}", duplicateDoc.getId(), duplicateDoc.getTitle());
                    // 如果是PDF文件，获取页数
                    int pageCount = 0;
                    ...
                    return new FileUploadResult(existingFileName, pageCount, 
                        duplicateDoc.getId(), duplicateDoc.getTitle(), duplicateDoc.getPicUrl());
                }
            }
